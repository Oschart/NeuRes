...
def get_config():
    ...
    parser.add_argument("--batch_size", type=int, default=1,
                        help="Number of formulas per forward pass. Effective formulas/optimizer step = update_every * batch_size.")
    ...
    # Mixed-precision autocast for the per-step forward (bf16 = 1.5–2x on Ampere+).
    # "none" preserves fp32 behavior; "bf16" recommended (no grad scaler needed).
    ...
    config['lr_schedule'] = parse_lr(config['lr_schedule'])

    if config['freeze'] != "":
        config['freeze'] = config['freeze'].split(",")
    else:
        config['freeze'] = []

    if not config['no_wandb'] and config['wandb_key'] is None:
