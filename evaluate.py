...
class ModelTest:
	def __init__(self, rank, world_size):
		...
		config = update_config(get_config(), model_opts)
		config['dataset'] = test_opt.dataset
		config['eval_dir'] = test_opt.eval_dir
		config['test_only'] = True
		# th.cuda.set_device(config['device'])
		print(f"Testing model: {self.model_name}")
		print("Creating env...")
		print(f"Opts: {config}")
		env = ResUNSAT(config, shuffle=False)
		pool_size = len(env.dataset["test"]["problems"])
		print(f"Total test pool = {pool_size}")
		
		print("Creating runner...")
		runner = TeacherForce(config, env)
		...
		os.makedirs(f"{config['eval_dir']}/{config['dataset']}/", exist_ok=True)
		pkl.dump(res, open(f"{config['eval_dir']}/{config['dataset']}/{self.model_name}{addendum}.pkl", "wb"))
